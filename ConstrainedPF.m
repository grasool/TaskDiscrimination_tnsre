% Use Kalman filter/Particel Filter to estiamte the intended movements


function ConstrainedPF()
close all; clear; clc;

% Put data in a structure
sData.FileName     = 'SID1_UT_1DOF';       % File to process
sData.nSynergy     = 3;                    % number of synergies
sData.cTp          = 0.7;                  % contration time percentage
sData.nRealizations= 2;                    % number of relaizations
...
%% Main function for movment identification, Synergy Extraction and Kalman Filering
...
for i=1:nRealizations                    
    ...
    try                 % If there is error in synergy extraction, catch it and process ir
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % Get synergy matrices (Training) using pICA
        % We can use NMF
        ...
        AverageAccuracy(i)  = fFiltering( synergyMatrices, TrainingData, ValidationData);                   % Kalman Filtering
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
function avAccuracy = fFiltering(sMatrices, TrainingData, ValidationData)

% 8 channels
nChannel = 8;
Measurements = ValidationData(1:nChannel,:);         % Select only RMS values ignore other features
...
for j = 1:nMeasurements
    for k=1:nMovements
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % We need to change this function so that we have state estimate
        % from the Constrained PF. We donot need Vnnt (covariance estiamtion matrix)
        
        [Xnnt(:,k), Vnnt(:,:,k) ] = fKalmanFilter(Xnn, Vnn, sMatrices(:,:,k), Measurements(:,j));
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        
        
        
        %%% The similarity measure may not need modification.
        % Get the similarity using the state estimate
...
% Perfrom LDA
[~, classLDA, probLDA] = fLDAClassify( TrainingData, ValidationData, nMovements );
...
for j=1:sigTreated.nM
    for i = 1:sigTreated.nCh
        ...
        FeatureZC = ZCfeature(Selected_Data);                   % ZC
        FeatureWL = sum(abs(diff(Selected_Data)));              % Waveform Length
        FeatureSSC = SSCfeature(Selected_Data);
...
function [X_n_n, V_n_n] = fKalmanFilter (X_n_n, V_n_n, H_n, y_n)
...
function ZC = ZCfeature(DataMatrix)
...
function SSC = SSCfeature(DataMatrix)
